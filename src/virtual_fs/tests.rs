...
/// A remote *update* of a file with open handles is deferred: `update_remote_file`
/// refuses to rotate the inode's identity while any handle is open (mirrors the
/// deletion path's open-handle guard), so no kernel invalidation fires for it this
/// cycle — the update is re-detected and applied once the last handle closes. This
/// avoids both a chimeric flush against a rotated frame and the page-invalidation
/// deadlock that #195 guarded against (we issue no invalidation at all here). A
/// file with no open handle rotates immediately and gets `Pages`.
#[test]
fn poll_update_deferred_for_open_handles() {
    let hub = MockHub::new_repo();
    ...
    rt.block_on(async {
        ...
        let recorded = calls.lock().unwrap().clone();
        assert!(
            !recorded.iter().any(|&(ino, _)| ino == open_attr.ino),
            "updated open inode must not be invalidated: rotation is deferred while a handle is open (got {:?})",
            recorded,
        );
        assert!(
            recorded.contains(&(closed_attr.ino, InvalKind::Pages)),
